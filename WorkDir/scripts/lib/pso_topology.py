"""
PSO placement topology — stores (x, y) solution coordinates as state.
...
for cost normalization. PSOOptimizer.run() replaces state with the PSO solution
via set_solution().

Structurally identical to ILPTopology; kept separate so the compatibility registry
can distinguish the two optimizers.
"""
...
class PSOTopology(TopologyBase):
    """
    Coordinate-space topology for PSO placement.

    State is {bid: (x, y)} positions + {bid: variant_idx}.
    seed() produces a row-packed initial layout. PSOOptimizer replaces
    state with the swarm solution via set_solution().
    """
    ...
    def seed(self, blocks: dict, mode: str = "random") -> None:
        # mode ignored — PSOOptimizer replaces all positions via set_solution().
        # Row-pack only provides the pipeline's initial decode() reference for cost normalization.
        ...
        logger.debug("PSOTopology seeded %d blocks in row-pack layout", len(valid))
